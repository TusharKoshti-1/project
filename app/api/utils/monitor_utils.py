...
def monitoring_loop(employee_id: int, db: Session):
    """
    Monitors the employee by processing frames from the webcam.
    Stops when the stop_event is set (e.g., on logout) and releases the camera.
    """
    camera = get_camera()
    ...
    try:
        ...
        # Ensure the camera is released when the loop stops
        logger.info(f"Monitoring stopped for employee_id: {employee_id}")
        release_camera()  # Close the webcam
...
def start_recognition_and_monitoring(
    ...
    """
    Starts the recognition process to identify the employee and begins monitoring.
    Ensures the camera is released if recognition fails.
    """
    camera = get_camera()
    if not camera or not camera.cap:
    ...
    try:
        while (datetime.datetime.now() - start_time).seconds < timeout:
            ...
            if frame_count % frame_skip == 0:
                if recognition_service.recognize_user(frame, login_id):
        ...
        # Ensure the camera is released and reset after recognition attempt
        release_camera()
        logger.info("Camera released after recognition attempt")
